/* global mcsAdmin */
(function (wp) {
	if (!wp || !document.getElementById('modern-coming-soon-admin')) {
		return;
	}
	...
	apiFetch.use(apiFetch.createNonceMiddleware(mcsAdmin.nonce));

	const isFa = (mcsAdmin?.locale || '').toLowerCase().startsWith('fa');
		...
		h(Card, null,
			...
			h(CardBody, null,
				h('div', { className: 'mcs-toggle-grid' },
					h(ToggleControl, {
						label: T('RTL layout', 'چیدمان راست‌چین'),
						checked: settings?.typography?.rtl,
						onChange: (checked) => setSettings({ ...settings, typography: { ...settings.typography, rtl: checked } }),
					}),
					h(ToggleControl, {
						label: T('Countdown', 'شمارش معکوس'),
						checked: settings?.countdown?.enabled,
						onChange: (checked) => setSettings({ ...settings, countdown: { ...settings.countdown, enabled: checked } }),
					}),
					h(ToggleControl, {
						label: T('Subscription form', 'فرم اشتراک'),
						checked: settings?.sections?.subscribe,
						onChange: (checked) => setSettings({ ...settings, sections: { ...settings.sections, subscribe: checked } }),
					}),
					h(ToggleControl, {
						label: T('Noindex', 'نوایندکس'),
						checked: settings?.noindex,
						onChange: (checked) => setSettings({ ...settings, noindex: checked }),
					})
				)
			)
		),
		...
		h(Card, null,
			...
			h(CardBody, null,
				h('div', { className: 'mcs-stats' },
					h('div', null, h('strong', null, (mcsAdmin.subscribers?.total) || 0), h('div', null, T('Subscribers', 'مشترکین'))),
	...
	function Design({ settings, setSettings, previewUrl }) {
			...
			h('div', { className: 'mcs-design__controls' },
				h(Card, null,
					...
					h(CardBody, null,
						...
						h(TextareaControl, {
							label: __('Description', 'modern-coming-soon'),
				...
				h(Card, null,
					...
					h(CardBody, null,
						h('div', { className: 'mcs-color-row' },
							h('div', null,
								h('div', null, __('Background', 'modern-coming-soon')),
								h(ColorPicker, {
									color: settings?.background?.value || '#101010',
									onChangeComplete: (v) => update({ background: { ...settings.background, value: v.hex } }),
								})
							),
							h('div', null,
								h('div', null, __('Button color', 'modern-coming-soon')),
								h(ColorPicker, {
									color: settings?.button_color || '#0ea5e9',
									onChangeComplete: (v) => update({ button_color: v.hex }),
								})
							)
						)
					)
				...
				h(Card, null,
					...
					h(CardBody, null,
						h(TextControl, {
							label: __('Font family', 'modern-coming-soon'),
							value: settings?.typography?.font_family || '',
							onChange: (val) => updateSection('typography', 'font_family', val),
						}),
						h(ToggleControl, {
							label: __('RTL', 'modern-coming-soon'),
							checked: settings?.typography?.rtl,
							onChange: (checked) => updateSection('typography', 'rtl', checked),
						}),
						h(RangeControl, {
							label: __('Title size', 'modern-coming-soon'),
							value: settings?.title_size || 40,
							min: 20,
							max: 72,
							onChange: (val) => update({ title_size: val }),
						})
					)
				),
				h(Card, null,
					...
					h(CardBody, null,
						Object.entries(settings?.sections || {}).map(([key, val]) =>
							h(ToggleControl, {
								...
								label: key,
								checked: !!val,
								onChange: (checked) => updateSection('sections', key, checked),
							})
						)
					)
				),
				h(Card, null,
					...
					h(CardBody, null,
						h(TextControl, {
							label: __('Label', 'modern-coming-soon'),
							value: settings?.button_label || '',
							onChange: (val) => update({ button_label: val }),
						}),
						h(TextControl, {
							label: __('URL', 'modern-coming-soon'),
							value: settings?.button_url || '',
							onChange: (val) => update({ button_url: val }),
						})
					)
				),
				h(Card, null,
					...
					h(CardBody, null,
						h(ToggleControl, {
							label: __('Enable countdown', 'modern-coming-soon'),
							checked: settings?.countdown?.enabled,
							onChange: (checked) => updateSection('countdown', 'enabled', checked),
						}),
						h(TextControl, {
							label: __('Target (YYYY-MM-DD HH:MM)', 'modern-coming-soon'),
							value: settings?.countdown?.date || '',
							onChange: (val) => updateSection('countdown', 'date', val),
						})
					)
				)
			),
		h('div', { className: 'mcs-design__preview' },
			...
			h('div', { className: 'mcs-preview-shell' },
				h('iframe', {
					ref: previewRef,
					title: __('Preview', 'modern-coming-soon'),
					src: previewUrl,
					className: 'mcs-preview-frame',
					style: { transform: `scale(${scale})`, transformOrigin: 'top center', width: `${100 / scale}%`, height: `${700 / scale}px` },
				})
			)
		)
	);
...
function Templates({ settings, setSettings, pluginUrl, previewUrl }) {
			...
			h('div', { className: 'mcs-template-filters' },
				h('div', { className: 'mcs-chips' },
					categories.map((cat) =>
						h(Button, {
							key: cat,
							isPrimary: cat === category,
							isTertiary: cat !== category,
				...
				h(SearchControl, {
					value: search,
					onChange: setSearch,
					placeholder: __('Search templates', 'modern-coming-soon'),
				})
			),
			loading ? h(Spinner, null) :
				h('div', { className: 'mcs-template-grid' },
					filtered.map((tpl) =>
						h('div', {
							key: tpl.slug,
							className: 'mcs-template-card ' + (settings?.template === tpl.slug ? 'is-selected' : ''),
							onClick: () => setSettings({ ...settings, template: tpl.slug, custom_html: '' }),
						},
							(() => {
								const thumb = (tpl.thumbnail && tpl.thumbnail.startsWith('http')) ? tpl.thumbnail : pluginUrl + (tpl.thumbnail || tpl.screenshot || '');
								return h('div', {
									className: 'mcs-template-thumb',
	...
	function AccessRules({ settings, setSettings }) {
		...
		return h('div', { className: 'mcs-grid' },
					...
					h('div', { className: 'mcs-role-list' },
						roles.map((role) =>
							h(CheckboxControl, {
								key: role.value,
								label: role.label,
								checked: (settings?.bypass_roles || []).includes(role.value),
					...
					h('div', { className: 'mcs-repeater' },
						ipList.map((item, idx) =>
							h('div', { className: 'mcs-repeater__row', key: idx },
								h(TextControl, {
									label: idx === 0 ? __('IP/CIDR', 'modern-coming-soon') : undefined,
									value: item,
									onChange: (val) => {
										const next = [...ipList];
										next[idx] = val;
										update({ bypass_ips: next });
									},
								}),
				...
				h(CardBody, null,
					h('div', { className: 'mcs-repeater' },
						urlList.map((item, idx) =>
							h('div', { className: 'mcs-repeater__row', key: idx },
								h(TextControl, {
									label: idx === 0 ? __('Path/Regex', 'modern-coming-soon') : undefined,
									value: item,
									onChange: (val) => {
										const next = [...urlList];
										next[idx] = val;
										update({ bypass_urls: next });
									},
								}),
			...
			h(Card, null,
				...
				h(CardBody, null,
					h(ToggleControl, {
						label: __('Enable bypass token', 'modern-coming-soon'),
						checked: settings?.bypass_token_enabled,
						onChange: (checked) => update({ bypass_token_enabled: checked }),
					}),
					...
					tokenMessage && h(Notice, { status: 'info', isDismissible: true },
						h('div', { className: 'mcs-token-row' },
							...
							h(Button, {
								variant: 'secondary',
								onClick: () => {
									navigator.clipboard?.writeText(tokenMessage.replace(/^[^:]+:\s*/, ''));
								},
							}, __('Copy', 'modern-coming-soon'))
						)
					),
					showConfirm && h(Modal, {
						title: __('Regenerate token?', 'modern-coming-soon'),
			...
			h(Card, null,
				...
				h(CardBody, null,
					h(ToggleControl, {
						label: __('Block REST API', 'modern-coming-soon'),
						checked: settings?.block_rest_factory,
						onChange: (checked) => update({ block_rest_factory: checked }),
					}),
					h(ToggleControl, {
						label: __('Block XML-RPC', 'modern-coming-soon'),
						checked: settings?.block_xmlrpc,
						onChange: (checked) => update({ block_xmlrpc: checked }),
					})
				)
			)
		);
	...
	function Advanced({ settings, setSettings }) {
		...
		return h('div', { className: 'mcs-grid' },
			h(Card, null,
				...
				h(CardBody, null,
					h(TextControl, {
						label: __('Coming Soon status', 'modern-coming-soon'),
						value: settings?.status_code || 200,
						onChange: (val) => update({ status_code: parseInt(val, 10) || 200 }),
					}),
					h(TextControl, {
						label: __('Factory status', 'modern-coming-soon'),
						value: settings?.status_code_factory || 503,
						onChange: (val) => update({ status_code_factory: parseInt(val, 10) || 503 }),
					}),
					h(TextControl, {
						label: __('Retry-After (seconds)', 'modern-coming-soon'),
						value: settings?.retry_after || 0,
						onChange: (val) => update({ retry_after: parseInt(val, 10) || 0 }),
					})
				)
			),
			h(Card, null,
				...
				h(CardBody, null,
					h(ToggleControl, {
						label: __('Noindex', 'modern-coming-soon'),
						checked: settings?.noindex,
						onChange: (checked) => update({ noindex: checked }),
					}),
					h(TextareaControl, {
						label: __('Meta description', 'modern-coming-soon'),
						value: settings?.seo_description || '',
						onChange: (val) => update({ seo_description: val }),
					})
				)
			),
			h(Card, null,
	...
	function App() {
		...
		if (loading || !settings) {
			return h('div', { className: 'mcs-loading' }, h(Spinner, null));
		}

		let main = null;
		switch (tab) {
			case 'dashboard':
				main = h(Dashboard, { settings, setSettings });
				break;
			case 'design':
				main = h(Design, { settings, setSettings, previewUrl: previewUrlWithTemplate });
				break;
			case 'templates':
				main = h(Templates, { settings, setSettings, pluginUrl: data.pluginUrl, previewUrl: data.previewUrl });
				break;
			case 'access':
				main = h(AccessRules, { settings, setSettings });
				break;
			case 'subscribers':
				main = h(Subscribers, null);
				break;
			case 'advanced':
				main = h(Advanced, { settings, setSettings });
				break;
			default:
				main = null;
		}
		...
		return h('div', { className: 'mcs-admin-app ' + (data.isRTL ? 'is-rtl' : '') },
			h(HeaderBar, {
				...
				onSave: save,
				saving,
				previewUrl: previewUrlWithTemplate,
			}),
	...
	wp.element.render(h(App), document.getElementById('modern-coming-soon-admin'));
})(window.wp);
